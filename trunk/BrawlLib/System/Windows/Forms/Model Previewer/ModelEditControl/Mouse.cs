    ...
    public partial class ModelEditControl : UserControl
    {
        ...
        private void modelPanel1_MouseDown(object sender, MouseEventArgs e)
        {
                ...
                if (bone != null)
                {
                    ...
                    if (GetOrbPoint(new Vector2(e.X, e.Y), out point))
                    {
                        ...
                        if (_rotating || _translating || _scaling)
                            BoneChange(pnlBones.SelectedBone, true);
                    }
                }
            ...
            bool ok = false;
                ...
                if (VertexLoc != null)
                {
                    ...
                    if (GetVertexOrbPoint(new Vector2(e.X, e.Y), ((Vector3)VertexLoc), out point))
                    {
                        ...
                        VertexChange(_selectedVertices, true);
                    }
                }
                else 
                    ok = true;
            ...
            GetVertex:
                ...
                //if (!(_snapX || _snapY || _snapZ || _snapCirc) && modelPanel1._selecting)
                //{
                //    ARGBPixel p = modelPanel1.DoPicking(e.X, e.Y);
                //    int objectIndex = p.R | (p.G << 8);
                //    int modelIndex = p.B | (p.A << 8);
                //    if (modelIndex > -1 && modelIndex < _targetModels.Count)
                //    {
                //        MDL0Node n = _targetModels[modelIndex];
                //        if (n._polyList != null && objectIndex > -1 && objectIndex < n._polyList.Count)
                //            pnlAssets.TargetObject = n._polyList[objectIndex] as MDL0ObjectNode;
                //    }
                //}
            }
        }
        ...
        private void modelPanel1_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == Forms.MouseButtons.Left)
            {
                ...
                if (_rotating || _translating || _scaling)
                    if (VertexLoc == null)
                        BoneChange(pnlBones.SelectedBone, false);
                    else
                        VertexChange(_selectedVertices, false);
        ...
        GetVertex:
            ...
            if (RenderVertices)
            {
                ...
                else
                {
                    ...
                    {
                        if (/*_hiVertex != TargetVertex && */!_hiVertex._selected)
                        {
                            _hiVertex._highlightColor = Color.Transparent;
                            modelPanel._forceNoSelection = false;
                        }
