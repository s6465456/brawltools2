        ...
        public override void OnPopulate()
        {
            for (int i = 0; i < 7; i++)
                if (Header->TextOffset(i) > 0)
                    new ModuleSectionNode() { _name = "Text" + i, _dataOffset = (int)Header->TextOffset(i), _dataSize = Header->TextSize(i) }.Initialize(this, (VoidPtr)Header + Header->TextOffset(i), (int)Header->TextSize(i));

            for (int i = 0; i < 11; i++)
                if (Header->DataOffset(i) > 0)
                    new ModuleSectionNode() { _name = "Data" + i, _dataOffset = (int)Header->DataOffset(i), _dataSize = Header->DataSize(i) }.Initialize(this, (VoidPtr)Header + Header->DataOffset(i), (int)Header->DataSize(i));
        }
    }
